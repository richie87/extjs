   ...
   //The following functions are used to get the sorting data from the toolbar and apply it to the store
    ...
    function changeSortDirection(button, changeDirection) {
        ...
        if (sortData) {
            if (changeDirection !== false) {
                ...
                button.setIconCls(Ext.String.toggle(iconCls, "sort-asc-" + theme, "sort-desc-" + theme));
            }
            store.clearFilter();
            doSort();
        }
    ...
    function createSorterButtonConfig(config) {
	    ...
	    var theme = Ext.themeName,
	        iconCls = Ext.String.format('sort-{0}-{1}', config.sortData.direction.toLowerCase(), theme);
    ...
    function createStyles(theme){
        var extension = (theme === 'crisp' || theme === 'neptune' || theme === 'neptune-touch') ? 'png' : 'gif',
            path = Ext.String.format('../../packages/ext-theme-{0}/build/resources/images/grid', theme);
            cssText = [
                '.sort-desc-{0} { background-image: url("{1}/sort_desc.{2}") !important; }',
                '.sort-asc-{0} { background-image: url("{1}/sort_asc.{2}") !important; }'
            ].join('');
	   
        return Ext.util.CSS.createStyleSheet(Ext.String.format(cssText, theme, path, extension), 'filters-buttons');
    }
